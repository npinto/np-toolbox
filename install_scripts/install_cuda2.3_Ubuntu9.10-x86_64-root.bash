...
wget http://developer.download.nvidia.com/compute/cuda/2_3/drivers/cudadriver_2.3_linux_64_190.18.run
kill -9 $(ps aux | grep /usr/bin/X) 2> /dev/null
sh cudadriver_2.3_linux_64_190.18.run --ui=none --no-questions --accept-license -X
...
sh ./cudatoolkit_2.3_linux_64_ubuntu9.04.run --noexec --target cudatoolkit/
...
cp -v /home/thor/install/cuda23/cudasdk_2.3_linux.run $TMP_DIR
chmod -vR a+rwx $TMP_DIR
sudo -u thor sh $TMP_DIR/cudasdk_2.3_linux.run --noexec --target cudasdk/
